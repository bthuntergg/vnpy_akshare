...
def QA_fetch_get_stock_day(code, start_date, end_date, if_fq='00', level='day', ip=best_ip, port=7709):
    ...
    with api.connect(ip, port):
        ...
        data = pd.concat(
            [api.to_df(api.get_security_bars(level, select_market_code(code), code, (9 - i) * 800, 800)) for i in
             range(10)], axis=0)
...
def QA_fetch_get_stock_min(code, start, end, level='1min', ip=best_ip, port=7709):
    ...
    with api.connect(ip, port):
        ...
        data = pd.concat([api.to_df(api.get_security_bars(level, select_market_code(
...
def QA_fetch_get_stock_latest(code, ip=best_ip, port=7709):
    ...
    with api.connect(ip, port):
        data = pd.concat([api.to_df(api.get_security_bars(
            9, select_market_code(item), item, 0, 1)).assign(code=item) for item in code], axis=0)
        return data \
    ...
    with api.connect(ip, port):
        ...
        for id_ in range(int(len(code) / 80) + 1):
            __data = __data.append(api.to_df(api.get_security_quotes(
                [(select_market_code(x), x) for x in code[80 * id_:80 * (id_ + 1)]])))
...
def __QA_fetch_get_stock_transaction(code, day, retry, api):
    data_ = pd.concat([api.to_df(api.get_history_transaction_data(
        select_market_code(str(code)), str(code), (20 - i) * 800, 800, QA_util_date_str2int(day))) for i in range(21)],
        axis=0)
    ...
    with api.connect(ip, port):
        ...
        for index_ in range(trade_date_sse.index(real_start), trade_date_sse.index(real_end) + 1):
            ...
            try:
                ...
                print('Wrong in Getting %s history transaction data in day %s' % (code, trade_date_sse[index_]))
                # QA_util_log_info('Wrong in Getting %s history transaction data in day %s' % (code, trade_date_sse[index_]))
            else:
                print('Successfully Getting %s history transaction data in day %s' % (code, trade_date_sse[index_]))
...
class TdxDatafeed(BaseDatafeed):
    ...
    def init(self, output: Callable = print) -> bool:
        ...
        #     output("Tdx数据服务初始化失败：用户名为空！")
        #     return False
        #
        # if not self.password:
        #     output("Tdx数据服务初始化失败：密码为空！")
        #     return False

        # ts.set_token(self.password)
        # self.pro: Optional[DataApi] = ts.pro_api()    删除

        self.pro: Optional[TdxHq_API] = TdxHq_API()
        self.inited = True
    ...
    def query_bar_history(self, req: HistoryRequest, output: Callable = print) -> Optional[List[BarData]]:
        ...
        #     return None
        #
        level: str = INTERVAL_VT2TDX.get(interval)
        if not level:
            return None
        ...
        try:
            ...
            with api.connect(best_ip, 7709):
                ...
                d1: DataFrame = pd.concat(
                    [api.to_df(api.get_security_bars(level, select_market_code(code), code, (9 - i) * 800, 800)) for i
                     in range(10)], axis=0)

                d1 = d1.assign(datetime=pd.to_datetime(d1['datetime']), code=str(code)) \
                         .drop(['year', 'month', 'day', 'hour', 'minute'], axis=1, inplace=False) \
                         .assign(date=d1['datetime'].apply(lambda x: str(x)[0:10])) \
                         .assign(date_stamp=d1['datetime'].apply(lambda x: QA_util_date_stamp(x))) \
                         .assign(time_stamp=d1['datetime'].apply(lambda x: QA_util_time_stamp(x))) \
                         .assign(type=type_).set_index('datetime', drop=False, inplace=False)[start:end]

                d1 = d1.assign(datetime=d1['datetime'].apply(lambda x: str(x)))
        ...
        if df is not None:
            for ix, row in df.iterrows():
                ...
                if interval.value == "d":
                    dt: str = row["date"]
                    dt: datetime = datetime.datetime.strptime(dt, "%Y-%m-%d")
                else:
                    dt: str = row["time_stamp"]
                    dt: datetime = datetime.datetime.fromtimestamp(dt)- adjustment
                    # dt: datetime = datetime.datetime.strptime(dt, "%Y-%m-%d %H:%M:%S") - adjustment
